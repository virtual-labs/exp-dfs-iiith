...
function reset() {
  ...
  document.getElementById('ins').innerHTML = 'Execution is reset, select a source node for DFS'
}
class dfs_class {
  constructor() {
    this.first = 0
...
graph.on('click', function (params) {
  ...
  if (dfs_practice.first == 0) {
    ...
    document.getElementById('ins').innerHTML =
      'Now perform DFS with source node <b>' + params['nodes']['0'] + '</b>' + ', select an unvisited child of <b>'+ params['nodes']['0'] + '</b>'
    ...
    if (dfs_practice.flagtocheck == 0) {
      dfs_practice.done = 1
      document.getElementById('ins').innerHTML +=
        '<br>As selected source node has no children ,DFS is done on the selected source node!!!'
    }
    ...
    if (g.finalvisited[params['nodes']['0']] == 1) {
      document.getElementById('ins').innerHTML =
        'Selected node is incorrect as it is already visited!!!'
    } else {
      dfs_practice.selected[dfs_practice.selectedcnt++] = params['nodes']['0']
      ...
      for (var i = 0; i < g.dfsarr[g.dfscount - 1].length; i++) {
        if (g.dfsarr[g.dfscount - 1][i] == params['nodes']['0']) {
          ...
          if (dfs_practice.flagtocheck == 0) {
            ...
            if (g.edges[params['nodes']['0']].length == 0) {
              document.getElementById('ins').innerHTML =
                'Selected node is a leaf node.<br>Trace back to ancestor with unvisited child and select the unvisited child.'
              ...
              for (var i = 0; i < g.edges[params['nodes']['0']].length; i++) {
                if (
                  g.finalvisited[g.edges[params['nodes']['0']][i].node] == 0
                ) {
                  dfs_practice.flag = 1
                }
              }
              if (dfs_practice.flag == 0) {
                document.getElementById('ins').innerHTML =
                  'Children of selected node are already visited,<br>so trace back to ancestor with unvisited child and select the unvisited child.'
                dfs_practice.temp = null
                dfs_practice.onleafnode = params['nodes']['0']
                clickednode(params['nodes']['0'])
                g.dfs_pra(params['nodes']['0'])
                ...
                document.getElementById('ins').innerHTML =
                  'Now select an unvisited child of <b>' +
                  params['nodes']['0'] +
                  '</b>'
                dfs_practice.temp = params['nodes']['0']
                clickednode(params['nodes']['0'])
        ...
        if (dfs_practice.checker == 0) {
          if (dfs_practice.temp != null)
            document.getElementById('ins').innerHTML =
              'Selected node is incorrect.<br>Select an unvisited child of <b>' +
              dfs_practice.temp +
              '</b>'
          else {
            document.getElementById('ins').innerHTML =
              'Selected node is incorrect.<br>Trace back to ancestor of <b>' +
              dfs_practice.onleafnode +
              '</b> with unvisited child and select the unvisited child.'
          }
        }
      }
    }
